package unit2;

import java.util.Scanner;

/**
 * Description: This program asks the user for a number, and then square roots it.
 * Date: November 22, 2024
 * @author Misha Buterin
 */
public class LoopChallenge2 {
/**
 * This is the entry point to the program.
 * @param args
 */
	public static void main(String[] args) {
		 /*
	      Write a program to take the square root of a number typed in by 
	      the user. Your program should use a loop to ensure that the number 
	      they typed in is positive. If the number is negative, you should 
	      print out some sort of warning and make them type it in again.
	      
	      You can get the square root of a number n with Math.sqrt(n). Make 
	      sure you don't do this until the loop is done and you know for 
	      sure you've got a positive number.
	      
	      Use the loop that you didn't use in LoopChallenge1
	    */
		
		Scanner sc = new Scanner(System.in);
		
		
		//Getting user input
		System.out.println("Welcome to the square root game!");
		System.out.print("\nPlease enter a number of your choice: ");
		int num = sc.nextInt();
		
		
		//Checking that their number isn't negative
		while (num < 0) {
			System.out.println("That is a negative number, it cannot be square rooted.");
			System.out.print("\nPlease enter another number: ");
			num = sc.nextInt();
		}
		
		//Square rooting and printing results
		System.out.println("\nThe square root of your number is " + Math.sqrt(num) + ".");
		
		sc.close();
	  }
	}